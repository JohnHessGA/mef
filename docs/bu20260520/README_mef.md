...
- **Fixed universe.** 305 stocks + 20 ETFs. No dynamic universe expansion in the near future. *(AFT-universe awareness, 2026-Q2: SHDB now publishes official AFT investable universes — `aft_investable_stocks_v1` with 666 active stocks and `aft_investable_etfs_v1` with 104 active ETFs — exposed via `shdb.investable_universe_member` and the bridge view `mart.aft_investable_pool_daily`. CCW is the first migrated consumer. MEF intentionally remains on its curated 305+20 universe; a future pass may compare MEF's universe against the official AFT universes for coverage gaps, but ranking and recommendations are unchanged.)*
